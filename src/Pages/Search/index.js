...
const SearchPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const productsData = await fetchDataFromApi(apiUrl);
        const productsArray = Array.isArray(productsData) ? productsData : [];

        let filteredByRoute = productsArray;

        if (catName) {
          filteredByRoute = filteredByRoute.filter(
            (product) =>
              product.category === catName || product.subCat === subcat
          );
        }
        ...
        const uniqueBrands = Array.from(
          new Set(filteredByRoute.map((product) => product.brand))
        );
    ...
    fetchProducts();
  }, [catName, subcat, location.search]);
  ...
  useEffect(() => {
    ...
    const filtered = products.filter((product) => {
      const matchesSubcat =
        subcats.length === 0 || subcats.includes(product.subCat);
      const matchesBrand =
    ...
    <>
      <section className="product_Listing_Page">
        <div className="productListing d-flex">
          <Sidebar brands={brands} onSubcatChange={handleSubcatChange} />{" "}
          {/* Pass brands and handleSubcatChange to Sidebar */}
